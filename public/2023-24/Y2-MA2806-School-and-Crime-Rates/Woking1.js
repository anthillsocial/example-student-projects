...
function preload() {
  dataset = loadTable("WokingSchools.csv", "header"); //load the data file as a table
}
...
function piechart(x, y, w, h) {
  let currentAngle = 16.5; //make a variable to keep track of what the starting angle of each sector arc should be
  ...
  for (let row = 0; row < dataset.getRowCount(); row++) {
    ...
    fill(245);
    textSize(24);
    text("Ofsted school reports in Woking", width/45 , 100);
