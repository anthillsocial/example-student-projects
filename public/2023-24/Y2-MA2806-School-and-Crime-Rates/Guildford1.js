...
function preload() {
  dataset = loadTable("GuildfordSchools.csv", "header"); //load the data file as a table
}
...
function piechart(x, y, w, h) {
  let currentAngle = 19; //make a variable to keep track of what the starting angle of each sector arc should be
  ...
  for (let row = 0; row < dataset.getRowCount(); row++) {
    ...
    fill(241, 5, 95);
    noStroke();
    rotate(sectorAngle / 1.2);
    text(dataset.getString(row, 0), w / 2 + 10, 0);
    pop();
    
    fill(245);
    textSize(24);
    text("Ofsted school reports in Guildford", width/45 , 100);
